...
export default function EditarHabitacionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false)
    router.push(`/admin/habitaciones/${id}`)
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
        <div className="flex items-center gap-3 sm:gap-4">
          <Button variant="ghost" size="sm" asChild>
            <Link href={`/admin/habitaciones/${id}`}>
              <ArrowLeft className="h-4 w-4 mr-2" />
              Volver
            </Link>
          </Button>
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-navy">Editar Habitación</h1>
            <p className="text-sm sm:text-base text-gray-600 mt-1">{room.name}</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
            ...
            <CardContent className="p-4 sm:p-6 pt-0 space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setRoom({ ...room, price_per_night: parseFloat(e.target.value) })}
                  required
                />
              </div>
              <div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-4 sm:mt-6">
          ...
          <Button type="button" variant="outline" asChild className="w-full sm:w-auto">
            <Link href={`/admin/habitaciones/${id}`}>Cancelar</Link>
          </Button>
        </div>
      </form>
    </div>
